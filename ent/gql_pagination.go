...
func (c *EpicConnection) build(nodes []*Epic, pager *epicPager, after *Cursor, first *int, before *Cursor, last *int) {
	...
	var nodeAt func(int) *Epic
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *Epic {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *Epic {
	...
	c.Edges = make([]*EpicEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &EpicEdge{
			Node:   node,
...
func WithEpicOrder(order *EpicOrder) EpicPaginateOption {
	if order == nil {
		order = DefaultEpicOrder
	}
	o := *order
	return func(pager *epicPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultEpicOrder.Field
		}
		pager.order = &o
		return nil
	}
...
func WithEpicFilter(filter func(*EpicQuery) (*EpicQuery, error)) EpicPaginateOption {
	return func(pager *epicPager) error {
		if filter == nil {
			return errors.New("EpicQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type epicPager struct {
	order  *EpicOrder
	filter func(*EpicQuery) (*EpicQuery, error)
}

func newEpicPager(opts []EpicPaginateOption) (*epicPager, error) {
	pager := &epicPager{}
	...
	if pager.order == nil {
		pager.order = DefaultEpicOrder
	}
	return pager, nil
}

func (p *epicPager) applyFilter(query *EpicQuery) (*EpicQuery, error) {
	if p.filter != nil {
...
func (p *epicPager) toCursor(e *Epic) Cursor {
	return p.order.Field.toCursor(e)
}

func (p *epicPager) applyCursors(query *EpicQuery, after, before *Cursor) *EpicQuery {
	for _, predicate := range cursorsToPredicates(
		p.order.Direction, after, before,
		p.order.Field.field, DefaultEpicOrder.Field.field,
	) {
...
func (p *epicPager) applyOrder(query *EpicQuery, reverse bool) *EpicQuery {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	query = query.Order(direction.orderFunc(p.order.Field.field))
	if p.order.Field != DefaultEpicOrder.Field {
		query = query.Order(direction.orderFunc(DefaultEpicOrder.Field.field))
	}
	return query
}

func (p *epicPager) orderExpr(reverse bool) sql.Querier {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	return sql.ExprFunc(func(b *sql.Builder) {
		...
		if p.order.Field != DefaultEpicOrder.Field {
			b.Comma().Ident(DefaultEpicOrder.Field.field).Pad().WriteString(string(direction))
		}
	})
}
...
func (e *EpicQuery) Paginate(
	...
	conn := &EpicConnection{Edges: []*EpicEdge{}}
