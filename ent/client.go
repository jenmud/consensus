...
import (
	...
	"github.com/jenmud/consensus/ent/migrate"
...
func (c *EpicClient) UpdateOne(e *Epic) *EpicUpdateOne {
	mutation := newEpicMutation(c.config, OpUpdateOne, withEpic(e))
...
func (c *ProjectClient) QueryReporter(pr *Project) *UserQuery {
...
// QueryAssignee queries the assignee edge of a Project.
func (c *ProjectClient) QueryAssignee(pr *Project) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := pr.ID
		step := sqlgraph.NewStep(
...
func (c *UserClient) QueryReporter(u *User) *ProjectQuery {
	query := (&ProjectClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
func (c *UserClient) QueryAssignee(u *User) *ProjectQuery {
	query := (&ProjectClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
