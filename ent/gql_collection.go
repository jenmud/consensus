...
func (e *EpicQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		switch field.Name {
		case "project":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &ProjectQuery{config: e.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
...
type epicPaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []EpicPaginateOption
}

func newEpicPaginateArgs(rv map[string]interface{}) *epicPaginateArgs {
	args := &epicPaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
	...
	if v, ok := rv[whereField].(*EpicWhereInput); ok {
		args.opts = append(args.opts, WithEpicFilter(v.Filter))
	}
	return args
}
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		...
		case "epics":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &EpicQuery{config: pr.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
